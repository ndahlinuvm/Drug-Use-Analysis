...
std_residuals(JI_vs_FA) |> 
  ...
  labs(x = "Used Heroin or Not",
       ...
       fill = "z-score for \nFA vs JI models")
```
With an extremely small P-value we reject our null hypothesis, and conclude that there is strong evidence that either Age or Heroin or both differ between different education levels. We need to include education level in our Age vs. Heroin use analysis. 
...
```
With a test statistic of 36.86 and 23 degrees of freedom the resulting P-value is .034. With a P-value < .05 we reject our null hypothesis that the relationship between Age and Heroin use is conditional on Education level. We have strong evidence that the odds ratios for heroin use by age at different levels of education are unequal to 1.
...
```
With a test statistic of 58.08 and 23 degrees of freedom the resulting P-value is ~0. With a P-value < .05 we reject our null hypothesis that the relationship between age and Heroin use is conditional on education level.
...
## Conclusion
<!-- Add overall conclusions for results below  -->
- Heroin 
reject on complete ind   meaning at least 2 associated 

reject on joint meaning  age/heroin use or both differ between different levels of education 

reject on conditional ind meaning odds ratios between age and heroin use change at different education levels  from (ie. One of the odds ratios isn't = to 1)

fail to reject homogeneous meaning even though we reject for conditional we fail to reject homogeneous meaning even though all the odds ratios may not be = to 1. We don't have enough evidence to say that they are not all = to each other.  
