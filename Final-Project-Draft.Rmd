...
drug_data_ex_old <- filter(drug_data, Age != "65+")

xtabs(formula = ~ has_used_Heroin + Education + Age,
      data = drug_data_ex_old) |> 
  ftable()
...
pchisq(q = cond_ind_test_Her,
       ...
       lower = F)
```
With a test statistic of 49.856 and 30 degrees of freedom the resulting P-value is .0128. With a P-value < .05 we reject our null hypothesis that the relationship between age and Heroin use is conditional on education level.
...
drug_data_ex_old<-mutate(drug_data_ex_old, college= ifelse(Education %in% c("Doctorate degree","Masters degree","Professional certificate/ diploma","University degree"), "Graduated", "Didn't Graduate"))
drug_data_ex_old |> 
  group_by(Age) |> 
...
drug_data <- mutate(drug_data, has_used_Meth = ifelse(Meth == "CL0", "Never", "Used"))
...
pchisq(q = cond_ind_test_Meth,
       ...
       lower = F)
```
With a test statistic of 57.239 and 30 degrees of freedom the resulting P-value is .00196. With a P-value < .05 we reject our null hypothesis that the relationship between age and Heroin use is conditional on education level.
