...
Age_Heroin_prop
# ggplot(data = Age_Heroin_prop,
#        mapping = aes(Age)) +
#   geom_bar(aes(fill = Heroin),
#            position = "dodge")

ggplot(Age_Heroin_prop, aes(x=Age,y=Freq, fill=Heroin))+
  geom_col()
...
Looking more heavily at Drug Use, Age, and Education.
...
I <- n_distinct(drug_data_comb_old$Education)
J <- n_distinct(drug_data_comb_old$has_used_Heroin)
K <- n_distinct(drug_data_comb_old$Age)

xtabs(formula = ~ has_used_Heroin + Education + Age,
      data = drug_data_comb_old) |> 
  ftable()
...
partial_chisq_tests_Her <- 
  ...
  summarize(test_stat = chisq_test(has_used_Heroin, Age)$statistic,
            df = chisq_test(has_used_Heroin, Age)$df,
            p_val = chisq_test(has_used_Heroin, Age)$p)
...
pchisq(q = cond_ind_test_Her,
       ...
       lower = F)
```
With a test statistic of 50.904 and 38 degrees of freedom the resulting P-value is .0787. With a P-value > .05 we fail to reject our null hypothesis that the relationship between Age and Heroin use is conditional on Education level, and conclude that the relationship between Age and Heroin use could be conditionally dependent on Education level.
...
drug_data_comb_old<-mutate(drug_data_ex_old, college= ifelse(Education %in% c("Doctorate degree","Masters degree","Professional certificate/ diploma","University degree"), "Graduated", "Didn't Graduate"))
drug_data_comb_old |> 
  group_by(Age) |> 
  
  summarize(below_5 = sum(chisq.test(has_used_Heroin, college)$expected <5))
...
heroin_used_df <- mutate(heroin_used_df, college= ifelse(Education %in% c("Doctorate degree","Masters degree","Professional certificate/ diploma","University degree"), "Graduated", "Didn't Graduate"))
...
temp_df <- filter(heroin_used_df,Age!="65+") 
...
I <- n_distinct(drug_data_comb_old$Education)
J <- n_distinct(drug_data_comb_old$has_used_Meth)
K <- n_distinct(drug_data_comb_old$Age)
...
xtabs(formula = ~ has_used_Meth + Age + Education,
      data = drug_data_comb_old) |> 
  ftable()
...
partial_chisq_tests_Meth <- 
  ...
  # Calculating the test statistic, df, and p-value for each individual partial table
  summarize(test_stat = chisq_test(has_used_Meth, Age)$statistic,
            df = chisq_test(has_used_Meth, Age)$df,
            p_val = chisq_test(has_used_Meth, Age)$p)
...
pchisq(q = cond_ind_test_Meth,
       ...
       lower = F)
```
With a test statistic of 60.909 and 38 degrees of freedom the resulting P-value is .0106. With a P-value < .05 we reject our null hypothesis that the relationship between age and Heroin use is conditional on education level.
...
drug_data_ex_old |> 
  ...
  summarize(below_5 = sum(chisq.test(has_used_Meth, college)$expected <5))
```
...
```{r Homogenous test}
#make education variable binary so we can run homogenous test
meth_used_df <- mutate(meth_used_df, college= ifelse(Education %in% c("Doctorate degree","Masters degree","Professional certificate/ diploma","University degree"), "Graduated", "Didn't Graduate"))
...
temp_df <- filter(meth_used_df,Age!="65+") 
#run Breslow Day test for association between age and education at different levels of drugs
drug_data_BDtest <- 
